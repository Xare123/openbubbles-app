...
internal object FaceTimeDiagnostics {
    ...
    internal fun formatStage(
        ...
        remoteLeave: FaceTimeRemoteLeaveEvidence? = null,
    ): String = FaceTimeDiagnosticPolicy.formatStage(stage, state, count, bytes, evidence, remoteLeave)
    ...
    internal fun logStage(
        ...
        try {
            ...
            if (log.record(stage, state, count, bytes, evidence, remoteLeave)) {
                Log.i(diagnosticTag, formatStage(stage, state, count, bytes, evidence, remoteLeave))
            }
        } catch (_: Exception) {
