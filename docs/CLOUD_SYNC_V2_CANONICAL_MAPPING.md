...
| `otr[part].lo/le` | `originalTextRange[part]` for projected edit parts | Validate decimal part keys, unsigned range arithmetic and collection bounds. Historical ranges do not index the current body; shortening is valid. `otr` can also describe untouched parts, so membership in `ep` is not required. Do not invent edits for those parts; their original metadata remains in protected source. | Native and ObjectBox tests pass; Windows restore verified; Pixel presentation and edit writing not qualified |
