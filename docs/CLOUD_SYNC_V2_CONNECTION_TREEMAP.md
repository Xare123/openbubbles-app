...
| Outbound durability before live write | The first durability repair now adds an all-or-none, scope- and generation-bound compare-and-swap renewal for the exact outbox batch. It renews both `leased` and ambiguity-fenced `unknownOutcome` rows through mapping preparation, native preflight, submission, response handling, and unknown-outcome reconciliation. Losing the renewal after submission fails the run without confirming or replaying the row. The engine, in-memory store, and shadow tripwire pass 185 focused local tests, including long-submission heartbeat and post-submission lease-loss cases; ObjectBox qualification remains pending on GCE because the ARM host lacks its native test DLL. The durable result still does not bind the complete server causality tuple; local message mutation and outbox admission are not one ObjectBox transaction; tombstone causality has no safe anti-resurrection proof; and account/generation fencing inside admission plus live legacy/V2 one-writer coexistence remain unproven. | Lease repair `TEST-PROVEN` locally; overall `NO-GO` for remote writes | Qualify the transactional ObjectBox renewal on GCE, then make local mutation plus outbox admission atomic. Keep remote writes disabled until result identity, tombstones, account fencing, and coexistence pass restart and ambiguity tests. |
