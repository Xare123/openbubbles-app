...
and device observations below remain dated evidence, not fresh live state.
 ...
 Next (current September 26): Pixel 57170DLCH000W8 shows the visible outbox-blocked Profile under parent diagnosis (setup finished, auth and UI ready, legacy off and inactive, outbox blocked, semantic pull unavailable). Notice fix 841bd3db2 on my branch makes blocked headline, action and button agree and refreshes on blocker change; service admission gates untouched. Alpha history conclusion is UNRESOLVED: package com.bluebubbles.messaging.alpha holds a 72MB ObjectBox store at app_flutter/objectbox/data.mdb; row counts and date range are unmeasured and no transfer is inferred. Restore Local stays prohibited as destructive. Zero VMs and runners remain. Accepted slices (2274fb44f display, 10cca2362 worker retry) unchanged; live-app display stays OPEN until the Pixel session runs.
