...
and device observations below remain dated evidence, not fresh live state.
 ...
 Next (current September 26): Pixel 57170DLCH000W8 live session checkpoint 1. Installed canary was an older same-version build and is now updated in place to staged f1f77d36 (1.15.0/20002227, hash verified, data preserved, no downgrade or clear). Fresh canary reports setup unfinished and auth not ready with UI ready; no repair or sign-in performed. Alpha is signed in with legacy sync off, but holds almost no local history on device (69 config entries, no message store), so a local export has nothing to transfer and Restore Local stays prohibited as destructive. No supported history archiver exists yet in the staged writer=false build. Remaining: sign-in plus live sync session, then the recorded Pixel plan. Zero VMs and runners remain. Accepted slices (2274fb44f display, 10cca2362 worker retry) unchanged; live-app display stays OPEN until the Pixel session runs.
