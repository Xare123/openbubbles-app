...
| Item | Current state |
| --- | --- |
| App branch | `agent/cloudkit-v2-update-seam` at `72335ddf5` plus the reviewed crash-recovery repair in the working tree. The next signed candidate must be built from the eventual repair commit, not from an older artifact. |
| Conditional-update executor candidate | **TEST-PROVEN, not installable yet:** the retained positive IDS receipt admits one exact version-checked update, preserves the original record identity/ETag, submits once, and reconciles only by exact readback. Independent review rejected signed run `34712211230` before installation because native receipts were acknowledged before ObjectBox finalized the readback. The working-tree repair now commits native evidence, clears the mutation fence, finalizes ObjectBox, and acknowledges receipts last. An injected acknowledgement failure proves the confirmed mapping and raw predecessor remain durable across executor restart. The focused five-file suite passes 291 tests; targeted analysis has no errors or warnings introduced by this repair. |
