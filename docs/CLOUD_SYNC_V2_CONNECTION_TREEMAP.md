...
| Candidate | Code `35551340c5dbd44ff05bec88ceec592ebbd12202` adds positive IDS acknowledgment proof to the lifecycle and attachment-integrity repairs. Completion alone previously included zero-target no-ops and APSError/TimedOut progress. The corrected Android and Windows paths require intended-participant acceptance before issuing V2 send proof. Prior live Android read proof remains `ad822f37cbf468a6bc74d602965e78ae02a852d1`. |
| Pending qualification | App Rust run `34439315172` and rustpush run `34439316836` target exact source `35551340c` on separate T2D-32 lanes, without APKs, writer controls, or automatic uploads. Execution and cleanup are pending. Locally, 27 composition contracts pass and Rust syntax checks pass; Dart analysis found no errors/warnings, with four unrelated existing style infos. |
...
| Dependency | rustpush `2bfbe8a06168a203c7c9f162b6fcbb45f1224560` adds a shared acknowledgment tracker above attachment-integrity base `a78ccfd`. Only explicit status 0 qualifies; the legacy meaning of 5008 is not assumed. Missing intended group targets remain unconfirmed. Ordinary send progress behavior is preserved. New tracker execution is pending. |
...
| Write-send provenance | `IN REPAIR` | Qualify the new native participant-acceptance gate and v2 protected receipt. Existing v1 receipts are retained but cannot replay/acknowledge. Pre-fix ready/adopted intents still require a reviewed provenance disposition before automatic uploads or production; changing the receipt decoder does not repair those rows. |
