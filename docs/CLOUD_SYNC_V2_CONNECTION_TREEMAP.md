...
| Exact local projection | Earlier signed Android state held 599 owned chats and projected 87 messages. The drained Windows profile holds 668 owned chats and 12,258 owned messages. The current Canary holds 176 legacy iMessage chat rows but zero messages because only one chat has an account- and generation-bound V2 ownership snapshot. Its first eligible message therefore stops at `canonical_identity_owner_unproven`. | Prior projection and current blocker `LIVE-PROVEN` | Acquire the independently authenticated owning Chat record from the chat-zone stream, then replay the retained Message under the same ownership resolver. A legacy row, message `chatID`, login state, or old cursor must never mint ownership. |
