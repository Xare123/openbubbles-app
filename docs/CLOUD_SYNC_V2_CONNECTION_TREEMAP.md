...
| Active date qualification / resume | Source `4e7121a18e8c011ae5472831111af86a61280178`, pilot 5fd8d03fe: Windows 34782347926 (job 103791533166) is building. GCE app-rust-only 34782416330 passed 631 native tests, including both date regressions; actual outcome JSON confirms success. Cleanup completed; independent inventories show zero VMs and no gce-34782416330-1 runner. No APK in this lane. First GCE 34782349481 rejected incompatible APK flags before creation. Next: verify/sign the Windows artifact, repeat exact retained date hashes, then replay. |
...
1. Qualify attachment timestamp source 4e7121a18 using the active exact-source
   native lanes. Verify/sign its DLL, run actual codec tests, then inspect offset
   256 and replay retained records. Multipart replies and raw-JPEG extension
   metadata already have separate restoration evidence; do not rerun them blindly.
