...
| Qualified source / next candidate | `16112ec6952dc953ce071c36ef79b203598e16d9` is Windows-qualified. Follow-up native decode-stage diagnostics are source-only; reply investigation is active. Do not label the newer working tree as a qualified runtime. Pixel integration remains separate. |
...
| Current native qualification | Windows run 34768626100 passed source `16112ec6952dc953ce071c36ef79b203598e16d9`, pilot `2343e13f9`, in 22m50s. 143 selected Rust tests, 519 focused Dart tests and 51 packaged-DLL codec tests passed. Parent verified 39 source inputs, 12 test logs, three ARM64 PEs and signed DLL `32b0bed6c53bc07ad034817627eb2a9cd5318fec859e4d9d93080b4d2a051c08`; 51 local codec cases and actual loaded-module path passed under App Control. Artifact lineage: private `artifacts/windows-native-34768626100/local-qualification.json`. |
...
| Next integration | The session-aware DLL is qualified, but five sampled type-2 archives fail metadata decoding as `Malformed`. Add fixed decode-stage evidence before changing validation; a bare association is not enough to restore a session. Two sampled type-3 records remain unsupported. Investigate normal replies rejected as ambiguous in parallel. |
| Active next run | Windows native-only run 34770635736, job 103759469072, source `008a342c566304c907249dae25721ca943164fe4`, pilot `a8db46f655150a1adac903c16abcbd613a23ea12`. Exact-source validation passed. Qualifies both diagnostic additions in one read-only batch, including five named native diagnostic cases and at least 25 extension cases. On success, verify/sign separately, rerun packaged codec tests, and inspect the same retained categories. Do not restart merely because a poll times out. No local build or live-profile process remains active. |
...
| Current read result | Windows report `obcs2-semantic-1789318496467790.json` applied two previously retained message records with zero fetched changes, outbox 17 -> 17 and empty terminal reads in all streams. Retained count is 6,648: 94 Chats, 5,344 Messages and 1,210 Attachments. Initial authentication refresh logged `cloud_sync_native_auth_refresh_session_missing`, so keep restart/auth recovery testing open despite the completed reads. Backup and report: private `build-evidence/windows-session-20260913`. Not a complete-history or two-new-distinct-messages claim. |
