...
and device observations below remain dated evidence, not fresh live state.
 ...
 Next (current September 25): run 36154397223 on 846afec86 ended CANCELLED by the workflow 60-minute job cap during the suite step, not Spot reclamation; zero VMs and runners remain. Per-suite outcomes are UNVERIFIED and no test verdict exists. Diagnostics fix a9e6f971f on pilot side branch agent/gce-runner-pilot-suite-evidence (per-suite status files, progress notices, always-run partial-log artifact, offline harness green) awaits review before one revised run. Last green suite step took under 8 minutes against 56 cancelled minutes; new tests show no obvious pending-Future defect on inspection. Accepted slices (2274fb44f display, 10cca2362 worker retry) unchanged; live-app display stays OPEN.
