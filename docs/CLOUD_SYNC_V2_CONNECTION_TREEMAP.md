...
| Pending qualification | Dart-only GCE run `34441590911` targets exact source `423a084250758246f7331501165b498651bcd34a` (code `731988a3d`) using isolated pilot `e4baad9ee`, T2D-32, with writer/automatic uploads off. Execution/cleanup pending. Local database tests compiled but setup failed because `objectbox.dll` was absent (126); no database test passed locally. 27 composition contracts pass; changed-file analysis has no errors/warnings and four unrelated existing style infos. Prior native base `35551340c` passed 377 app Rust and 260 rustpush tests, with cleanup independently verified before the new run. |
...
| Retained writer queue usability | `GAP` | Skipping pre-proof entries at lease selection does not prove the shared create-queue drain treats them as inert. Qualify the complete consumer path so safely deferred, never-submitted entries do not starve fresh sends; unknown outcomes must still reconcile, never be ignored. Do not silently discard retained old intents to make this test pass. |
