...
| Exact local projection | Earlier signed Android state held 599 owned chats and projected 87 messages. The preserved Windows profile holds 668 owned chats and 12,258 owned messages. The isolated compatibility replay recovered 311 additional historical iMessage messages and 133 attachment metadata rows, reaching 12,569 messages and 2,370 attachments. Every message has a chat owner and renderable content; outbox remains zero. A second drain applied zero rows. The read-only Windows list and detail viewers reached ready state in responsive signed windows, and earlier visual inspection confirmed real conversation rows without empty placeholders. Attachment metadata is projected during history catch-up; attachment bodies deliberately remain on demand. The production on-demand lane already composes account, generation, source, writer-exclusion, MMCS integrity, atomic-placement, and cleanup checks, but it has not yet completed a content-free live proof from this projection. | Windows projection, replay, and viewer `LIVE-PROVEN`; attachment-body composition `TEST-PROVEN`; Android presentation pending | Prove one small attachment through the existing on-demand lane in a disposable profile without logging its name, identifier, content, or key material. Then qualify one exact source and require the preserved Canary to show the same recovered history without a restart, orphan, blank row, duplicate, or write. |
...
It had zero messages without chat owners, zero messages without renderable
content, zero cross-kind ownership mismatches, and outbox zero. Recognized URL
counts rose with the recovered messages, while parser mismatch remained zero.
Rich link-preview payload projection and attachment-body materialization remain
separate incomplete features; this replay does not claim either is finished.
...
5. keep the 181 unresolved routes, link-preview payloads, and attachment bodies
   as separately measured backlog rather than hiding them inside a success
   claim.
