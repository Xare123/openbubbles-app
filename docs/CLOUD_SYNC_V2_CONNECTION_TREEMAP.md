...
- FIXED in qualified Windows test runtime (source baba20b16, run 35537790499, sample c78014c2): the same 8 UserInfoEmpty attachments now decode Ready; rejection count 8 to 0, Ready attachments 8 to 16, Ready total 23 (16 attachments, 4 messages, 3 reactions). Remaining dispositions persist (1 malformed-parent, 7 unsupported-type, 5 unsupported-service, 1 SMS). Decoder Ready is proven; body download, projection, and UI restoration are not. Next: exercise the protected materializer for one newly unblocked record under the bounded body-proof approval.
