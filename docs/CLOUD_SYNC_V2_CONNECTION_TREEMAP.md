...
| --- | --- | --- | --- |
| Windows private profile and identity | The isolated `cloudkit-v2-dev` profile and marker are present. A fresh offline schema-v4 inspection proves its drained local projection contains 668 chats, 12,258 messages, and 2,108 attachments. Its latest semantic operation report is still from an older dirty build, so it cannot qualify schema-v5 drain behavior. | Profile and local projection `LIVE-PROVEN`; current drain candidate unqualified | Build the corrected exact SHA from the clean detached Windows worktree while reusing only the isolated profile and shared compile cache. |
...
| Crash-recovery interlock feedback | A force-stop during a live legacy sync correctly releases the process and file locks but leaves the durable database fence valid for its five-minute crash-safety lease. Immediate legacy and V2 retries fail closed with only `cloudkit_interlock_busy`, even though no transfer is still running. Failed retries do not extend the lease. | Safety behavior `LIVE-PROVEN`; user guidance missing | Return a content-free busy reason plus bounded `retryAt` or remaining-wait metadata. Tell the user that the prior sync was interrupted and when one retry is safe; never clear or steal an unexpired fence. Add a force-stop/restart test proving the message counts down and acquisition succeeds after expiry. |
| Drain termination and launcher identity | The 16-pass outcome is now distinct resumable non-success. Every status is bound to a cryptographic launch ID and exact Dart PID, launchers are profile-serialized, and timeout or identity mismatch leaves the active exact process running. A later invocation rejects that retained harness before touching build artifacts and rechecks immediately before launch. The local-only projection viewer now uses a source-bound, artifact-hash build receipt for safe no-build restarts; a matching signed build restarted in 1.42 seconds, while mutated-artifact and changed-source receipt tests fail closed. CloudKit-capable no-build operations retain the stricter fresh semantic-report gate. The focused Dart and PowerShell behavioral suites pass; independent audit found no P0-P3 issue. | Viewer restart `LIVE-PROVEN`; drain terminal behavior `TEST-PROVEN` | Exercise exact status identity and either terminal-empty or resumable-cap behavior in the signed Windows harness. |
| Initial-sync workload controls | The first authenticated Android semantic pass used four pages of 50 changes per zone. It fetched 200 chat, 50 message, and 200 attachment changes; attachment metadata processing alone took about 65 seconds, while no message could project because the eligible message preceded its owning iMessage chat behind an SMS-heavy change stream. CloudKit continuation is an opaque per-zone token plus `moreComing`, so the client can resume but cannot safely date-seek. The installed Canary now offers one, four, or sixteen passes under one interlock and native-writer pause. Its current checkpoints are chat 474, message 50, and attachment 450. A content-free comparison with the already-drained Windows profile places its corresponding heads at 793, 18,992, and 3,626 changes. The comparison is directional rather than a cursor equivalence proof, but it shows why one small run cannot complete history and why sixteen passes can reach the chat head long before the message head. | Workload and resumability `LIVE-PROVEN`; bounded composition `TEST-PROVEN` | Run the existing four-pass Standard catch-up first. Require persisted reports after every pass and increasing chat ownership. Escalate to Deep only if Standard reaches its cap before the owning chat arrives. Do not fetch attachment bodies during initial history. |
...
| Exact local projection | Earlier signed Android state held 599 owned chats and projected 87 messages. The drained Windows profile holds 668 owned chats and 12,258 owned messages. All 12,258 have a chat owner and attributed body, 12,192 have plain text, 1,828 carry attachments, and none lack renderable content. A read-only Windows viewer rendered real conversation rows and a full message view twice without errors or empty placeholders. The current Canary still holds 176 legacy iMessage chat rows but zero messages because only one chat has an account- and generation-bound V2 ownership snapshot. Its first eligible message therefore stops at `canonical_identity_owner_unproven`. | Windows projection/UI and current Android blocker `LIVE-PROVEN` | Acquire the independently authenticated owning Chat record from the chat-zone stream, then replay the retained Message under the same ownership resolver. A legacy row, message `chatID`, login state, or old cursor must never mint ownership. |
| Android projection-to-UI handoff | `ChatsService` deliberately ignores its first zero-to-nonzero count transition and its incremental watcher adds only one chat when a transaction creates many. A CloudKit semantic transaction can do both, so a successful durable projection could leave the Messages screen empty or incomplete until restart. The manual Canary catch-up now performs one full asynchronous chat-list refresh only after the semantic session returns. A refresh failure cannot relabel the CloudKit operation as failed; it logs a content-free code and tells the user to restart to display newly available history. | Repair and focused source contract `TEST-PROVEN`; device proof pending | After Chat ownership catch-up projects messages, require the current Messages screen to populate without force-stop or restart. Also verify a refresh failure preserves the completed CloudKit report and gives the explicit restart fallback. |
| Canonical ownership bootstrap | Exact source `046bea639793163e3343e67c8367c44e8a08a526` passed the full GCE, signing, runner-cleanup, and native-library gates in run `33622711864`, then installed over the preserved Canary with Alpha unchanged. The corrected nullable-style bootstrap committed one authenticated Chat snapshot and three iMessage aliases, proving the circular bootstrap repair works. The next Message remained retained because its matching legacy Chat row has not yet been encountered as an authenticated V2 Chat record. A content-free census proves this is a missing-parent-order problem, not another nullable-style regression: current Canary has 176 iMessage chats and only one Chat snapshot; historical Android had 599 Chat snapshots before 87 Message snapshots; the drained Windows profile has all 668 Chat snapshots before 12,258 Message snapshots. | Bootstrap `LIVE-PROVEN`; owning Chat not yet fetched | Continue chat-zone catch-up before changing identity rules. Require Chat snapshots to rise above one, the retained Message to gain an independently proven owner, visible chats/messages in the actual UI, unchanged outbox `0 -> 0`, and unchanged Alpha state. |
...
3. The current Canary has the same 176 local iMessage chat shells visible in
   the Windows profile, but only one has a V2 Chat snapshot. Its eligible
   Message points to another shell. The resolver correctly refuses to treat a
   legacy row as proof that it belongs to the active Apple account and current
   checkpoint generation.
4. CloudKit Chat record names are generated identifiers, not a derivation of
...
5. The safe recovery is the same ordering that succeeded before: continue the
   opaque chat-zone token until the owning Chat record is authenticated, commit
   that owner and its aliases, then replay the retained Message. A message-
   derived owner, copied Windows snapshot, legacy `ckRecordId`, or relaxed
   account boundary would make the screen populate faster by accepting the
   wrong account's conversation and is rejected.

The next experiment is intentionally build-free: preserve Canary state and run
Standard catch-up. The current chat checkpoint is 474; the independently
drained Windows profile observed 793 chat changes. This does not prove the
tokens are interchangeable or that exactly 319 changes remain, but it predicts
that four 200-change windows should be enough to falsify the chat-first
hypothesis. Success requires more than a completed snackbar: Chat ownership
must increase, the retained Message must project, at least one chat/message
must be visible after an app restart, outbox must remain `0 -> 0`, and Alpha
...
2. all 12,258 have attributed-body content, 12,192 also have plain text, 1,828
   carry attachment relationships, and zero lack renderable content;
...
1. run the already-installed Canary's Standard catch-up without rebuilding;
   preserve every opaque token and require a persisted report after each pass;
2. inspect a read-only post-run database copy. Require Chat snapshots to rise
   above one and the retained Message's owner blocker to clear. If the run caps
   first, repeat from the durable checkpoint or use Deep catch-up;
3. restart Canary and require actual visible chats/messages. A fetched count,
   committed token, or successful component report alone is not completion;
4. continue resumable message history in bounded sessions. Keep SMS/MMS/RCS
   outside this release and defer attachment bodies until their Message owners
   exist;
5. only if live timing confirms that attachment-zone work materially delays
...
6. freeze one candidate SHA and qualify read plus create-only writer safety in
   CI before any live mutation;
7. send one double-confirmed plain-text Canary, then use the distinct
   confirmed-only replay lane as a zero-save idempotency proof; and
8. design token-expiry reset and broader attachment-body materialization as
