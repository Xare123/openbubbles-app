...
| Pre-digest ownership evidence | Signed Canary source `8518e6aa29dcb5b64a3094ff46ccc375cb1888c2` reached the preserved local journal but correctly stopped with `canonical_identity_owner_unproven`: snapshots committed by an older V2 build predated the two canonical-GUID ownership digests. The replacement does not weaken the global barrier. Before transport, it scans at most 4,096 exact applied saves in the current scope/generation, requires unique replay and current record-map provenance, re-decodes the protected row, requires byte-independent snapshot equality and an exact existing canonical Chat, Message, reaction, or Attachment identity, then changes only the two missing digest fields in one fenced transaction. A reaction parent or attachment owner must additionally carry exact durable Message-zone ownership in the same account and active dependency generation. Partial pairs, duplicate owners, stale or cross-scope dependencies, stale maps, changed immutable identity, or unsupported shapes remain blocking. | Repair `TEST-PROVEN`; signed live proof pending | Qualify the exact repair SHA, install it over Canary without clearing state, and run one pass. Require ownership-repair diagnostics before transport, no remaining ownership blocker for repairable rows, unchanged outbox `0 -> 0`, and unchanged Alpha package/state. |
