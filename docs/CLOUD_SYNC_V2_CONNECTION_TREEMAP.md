...
- TEST-PROVEN diagnostic / LIVE-OBSERVED rejection (source 2a65dfb58, run 35487733775, sample 4df5a8f7): the same 8 rejected attachments have nonempty raw ui dictionaries with no recognized pointer OR descriptive keys; four contain 3 unknown keys and four contain 4. All still fail UserInfoEmpty. The observer preserved durable sync state and disabled remote writes. Unknown metadata is not evidence that the file body is unavailable. MetadataOnlyUnsupportedMediaCredentials demotion remains rejected because it disables later download. Next: determine whether ui is optional ancillary metadata for the independent CloudKit lqa asset path, then review the smallest converter correction.
