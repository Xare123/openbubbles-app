...
| Pending qualification | Full GCE run `34437410835` targets exact source `75440cafc` with pilot `f8520b1ee`, T2D-60 in `us-west1-a`, manual writer/background read on and automatic uploads off. Creation is in progress; compilation, tests, APK, signing, and cleanup remain unverified. Prior attempt `34437303360` found no T2D-60 capacity in `us-west1-b`; no tests ran and cleanup/inventories confirmed no remaining VM or runner. Fix `4479546f4` passes 42 focused Dart/VM tests; the pilot's required-suite outcome gate passes 23 executable cases. |
