...
| Qualified source / next candidate | Native `e5547e8c7e9f1b4a4b243c408be84d099194867b` is Windows-qualified, including multipart numeric replies and chained-write tests. Raw plist data support for live-layout/icon fields is the next unqualified native change. Pixel integration remains separate. |
...
| Current native qualification | Run 34772982148 passed source `e5547e8c7e9f1b4a4b243c408be84d099194867b`, pilot `a3724ebd0`, in 26m9s: 148 selected Rust tests, 616 Dart tests, 51 packaged-DLL codec cases. Parent verified 45 source inputs, 12 logs, three ARM64 PEs and signed DLL `f05cf4387fbb2d3b1d29ecc500d491303aecc146ab766f4e7f890fa52c48ec9f`; 51 local codec cases and actual loaded path passed under App Control. Post-dispatch reply mapping/storage regressions passed the full 204-test local suite. |
| Next integration | Replies are live-proven below. Five sampled extension failures have direct plist data in liveLayoutInfo, not an NSData wrapper. Candidate accepts direct/wrapped bytes while keeping the 64 KiB live marker, compressed/decompressed icon limits and gzip validation. New bound labels distinguish string, depth, visit, byte and collection limits; no bounds increased. Qualify this candidate, then inspect/replay the same retained categories. |
| Active qualification / resume | Source `991b8379f38c35e3c2dfaa8a73308039cb9de0fd`, pilot `2da3562cea0b659571383a05be83dfec51079427`: Windows run 34775816810 (job 103773606609) and full GCE Canary run 34775818423 (create job 103773610019) both passed exact-source/configuration validation. GCE uses primary lane, t2d-standard-60, us-west1-b, existing 100 GiB SSD/75-minute lifetime, writer and automatic-upload controls enabled. Signing remains GitHub-hosted. No install until qualification/verification. Verify GCE cleanup and runner deregistration, including failure paths; expected runner name gce-34775818423-1. Do not duplicate a live run after polling timeout. |
...
1. Qualify direct-data extension decoding using the next exact native candidate.
   Verify/sign its DLL, run actual codec tests, then inspect and replay retained
   records. The qualified multipart-reply fix and its restoration evidence remain intact.
...
- Current local profile/test processes are closed; no agents remain active.
  ...
  C: had about 75 GiB free at the last checkpoint. Preserve the paired-copy
