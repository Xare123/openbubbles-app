...
| Source under qualification | `16112ec6952dc953ce071c36ef79b203598e16d9`: session-aware extension projection, late derived-content convergence, and the native logging-boundary correction. Current follow-up edits are documentation only. Pixel integration remains separate. |
...
| Active revised qualification | Windows run 34768626100 builds `16112ec6952dc953ce071c36ef79b203598e16d9` with pilot `2343e13f9`. Source validation passed; job remains active. Includes paged derived-content convergence and logging-boundary correction. Local full adapter suite: 154 passed. Do not use the previous failed session run as a qualified runtime. |
| Next integration | Qualify the session-aware DLL, then inspect retained records before a normal protected local projection pass. Type 2 now has a base dependency in source, but no live restoration proof yet. Type 4000/meta and 1000/no-balloon remain retained; do not claim full extension support. |
...
| Current read result | Windows report `obcs2-semantic-1789312307027272.json` applied four previously retained message records with zero fetched changes, outbox 17 -> 17 and empty terminal reads in all streams. Retained count is 6,650: 94 Chats, 5,346 Messages and 1,210 Attachments. Backup and report: private `build-evidence/windows-extension-20260913`. Not a complete-history or four-new-distinct-messages claim. |
| Actual extension boundary | The 37-record inspection kept durable state unchanged. Seven earlier extension failures now expose five `UnsupportedSticker` and two `UnsupportedAssociationType` results, so none of that seven is proved restored. Native get_cmd/receiver identifies type 2 as an extension-session update, not a generic sticker. Preserve its wire session grouping/base dependency before admitting it. Type 4000/meta and 1000/no-balloon remain separate. |
