...
and device observations below remain dated evidence, not fresh live state.
 ...
 Next (current September 26): Pixel 57170DLCH000W8 live session checkpoint 1. Installed canary was an older same-version build and is now updated in place to staged f1f77d36 (1.15.0/20002227, hash verified, data preserved, no downgrade or clear). Rami is now signed in (setup finished, auth and UI ready). Alpha history conclusion is UNRESOLVED: package com.bluebubbles.messaging.alpha holds a 72MB ObjectBox store at app_flutter/objectbox/data.mdb, so the earlier config-listing claim of no local history is withdrawn; row counts and date range are unmeasured and no transfer is inferred. Restore Local stays prohibited as destructive. No supported history archiver exists yet in the staged writer=false build. Remaining: sync/read validation plus the recorded Pixel plan. Zero VMs and runners remain. Accepted slices (2274fb44f display, 10cca2362 worker retry) unchanged; live-app display stays OPEN until the Pixel session runs.
