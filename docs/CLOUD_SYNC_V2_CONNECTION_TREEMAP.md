...
| Canonical ownership bootstrap | Exact source `e827179d854dee9326d1f65834444b43b13bf37a` passed the full GCE, signing, runner-cleanup, and native-library gates, then installed over the preserved Canary with Alpha unchanged. Its bounded live pull again stopped safely at `canonical_identity_owner_unproven` with writes disabled and outbox `0 -> 0`. A read-only copy of that exact post-pull ObjectBox store proved the active journal had 750 inbox rows but zero semantic snapshots, record maps, or replays. Source `d57fc0b5aeb3536f53dbf3f23b7005f767a72cbf` then passed every GCE/signing/cleanup gate and broke only that circular dependency, but its live pull safely quarantined the first chat as a conflict. The aggregate-only Windows inspector proved why: all 176 legacy Chat rows had exact composite GUID tails, nonblank matching identifiers, iMessage service, unique GUID ownership, and no cross-kind collision, but all 176 had nullable `style`. The corrected proof accepts only null legacy style or the exact decoded style; it still rejects contradictory non-null style, identifier/service drift, duplicate ownership, and cross-kind collision. The canonical upsert restores the decoded style inside the same transaction. A fixed-cutoff migration may requeue only the first current-generation, non-tombstone, protected save conflict under the active fence in read-only semantic mode; it preserves every source field and retry counter, and a failed retry completes after the cutoff so it cannot loop. | Bootstrap transaction `TEST-PROVEN`; nullable-style cause `LIVE-PROVEN`; corrected candidate analyzer clean | Run the corrected focused suites and exact-SHA GCE qualification. Install only the signed Canary artifact and require committed chat snapshots/maps/replays, visible local projection, no ownership blocker for exact legacy matches, unchanged outbox `0 -> 0`, and unchanged Alpha package/state. |
