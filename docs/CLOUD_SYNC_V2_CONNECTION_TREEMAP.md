...
| Relay activation and saved identity | Alpha's existing relay health check is green. The old manually entered sharing code is rejected by the registration relay with HTTP 401, while the iPhone relay bearer already saved in the isolated Windows profile returns HTTP 200 from the same version endpoint. The OABS/QR exporter deliberately supports only `MacOSConfig`; `RelayConfig` has no encoded transfer payload. A Windows `hw_info.plist` also contains a platform-keystore-bound NGM identity and cannot be treated as a portable Android login bundle. | `LIVE-PROVEN` state distinction; repaired Canary input pending signed build | Install the exact signed Canary containing the relay-input repair, atomically enter the live Windows-profile relay code, prove the staged iPhone identity, then complete Apple login without reading or changing Alpha state. |
...
| Candidate qualification | GCE run `33559197136` tested exact source `4b308b9085785c4346514b0b8a1aef157a0bf4b7`: generated bindings, full Dart, Rust, rustpush, protector, native APK verification, GitHub-hosted signing, runner deregistration, and VM deletion passed in 25m38s. That signed Canary is the installed baseline. The relay-input repair is newer and therefore requires its own exact-SHA qualification. | `LIVE-PROVEN` baseline; current relay repair unqualified | Freeze the relay-repair SHA and rerun the same signed Canary gate before in-place installation. |
