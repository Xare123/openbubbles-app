...
| Pending qualification | GCE run `34427563744`, full Canary on T2D-60, targets `68d5958b5` using isolated pilot `a35bfc526`. Configuration validation passed and runner creation started. Manual writer and background-read controls are enabled, automatic uploads remain off. The full lane now also requires Android JVM test execution and retains its XML evidence. Build, signing, and cleanup are not yet proven for this run. |
