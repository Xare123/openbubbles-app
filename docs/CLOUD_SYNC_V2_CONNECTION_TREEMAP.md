...
- Boundary traced: all 8 malformed attachment cases carry exactly native_attachment_conversion_user_info_empty, fired when the stored ui dict has none of the six pointer keys. Serde ignores unknown fields, so typed-None does not prove the stored dict lacked those keys; ui lives in cm metadata (guid checks passed before the user_info stage), so the distinction now needed is raw key shape, not paging. Unclassified shape: tests cover all-None and malformed-pointer variants but not descriptive-only metadata without pointers; absent ui converts Ready while present pointer-less ui quarantines. Demotion to MetadataOnlyUnsupportedMediaCredentials (already used for zero-byte and absent-size bodies) is the consistent candidate; the ALL_ASSETS materializer stays the authority on actual downloadability. Repair needs review and regression tests before runtime enablement.
