...
| Initial-sync workload controls | The first authenticated Android semantic pass used four pages of 50 changes per zone. It fetched 200 chat, 50 message, and 200 attachment changes; attachment metadata processing alone took about 65 seconds, while no message could project because the eligible message preceded its owning iMessage chat behind an SMS-heavy change stream. CloudKit's documented continuation model is an opaque per-zone server change token plus `moreComing`, so a client may pause at a durable token but cannot safely seek to an arbitrary date without risking skipped changes. The Android composition now offers one, four, or sixteen passes, preserving one interlock and native-writer pause across the selected bounded catch-up. The VM trigger retains its one-pass default and adds an explicit maximum catch-up mode. Fifty-five focused safety, controller, composition, and trigger-contract tests pass. | Workload need `LIVE-PROVEN`; bounded Android composition `TEST-PROVEN` | Qualify and run the sixteen-pass mode against the preserved Canary profile, prove every intermediate report is persisted, and verify a capped run resumes from its saved token. Keep attachment bodies out of initial catch-up; later production scheduling should use wall-time, battery, network, and thermal budgets around the same durable per-zone checkpoints. |
...
| Canonical ownership bootstrap | Exact source `e827179d854dee9326d1f65834444b43b13bf37a` passed the full GCE, signing, runner-cleanup, and native-library gates, then installed over the preserved Canary with Alpha unchanged. Its bounded live pull again stopped safely at `canonical_identity_owner_unproven` with writes disabled and outbox `0 -> 0`. A read-only copy of that exact post-pull ObjectBox store proved the active journal has 750 inbox rows but zero semantic snapshots, record maps, or replays. Therefore no pre-digest snapshot is repairable: the first decoder-ready Chat collides with an already-present legacy canonical Chat, while the ownership guard requires a durable semantic owner that cannot exist until that first apply commits. The candidate breaks only this circular dependency. After the ordinary global ownership check fails, it admits an existing row solely through the existing exact immutable-identity proof, writes a provisional ownership-bound snapshot inside the same ObjectBox transaction, reruns the complete global barrier, and only then permits canonical mutation. Any mismatch, unrelated incomplete owner, duplicate owner, record-map failure, missing terminal outcome, or later failure rolls back the proof and mutation together. New canonical rows and established semantic owners retain the normal path. | Root cause `LIVE-PROVEN`; candidate analyzer clean, executable tests pending | Run the focused gateway suite on GCE, then the full exact-SHA qualification. Install only the signed Canary artifact and require committed chat snapshots/maps/replays, visible local projection, no ownership blocker for exact legacy matches, unchanged outbox `0 -> 0`, and unchanged Alpha package/state. |
...
| Candidate qualification | GCE run `33591684465` tested exact source `6bb6a506db2f525a756d4e773fe1c98884bbcc05`: generated bindings, full Dart, Rust, rustpush, protector, native APK verification, GitHub-hosted signing, runner deregistration, and VM deletion passed in about 24 minutes. The signed Canary was installed in place without clearing its data and proved fresh relay admission, Alpha-parity Apple login, trusted-device PCS bootstrap, and one read-only 450-change semantic pass. The newer Android bounded drain, dedicated PCS preparation, and UX changes pass focused analysis and 55 tests but remain unqualified as an artifact. | Exact installed baseline `LIVE-PROVEN`; current working candidate locally `TEST-PROVEN` | Freeze the new SHA and rerun the same signed Canary gate before in-place installation. |
...
5. send one double-confirmed plain-text Canary, then use the distinct
   confirmed-only replay lane as a zero-save idempotency proof;
