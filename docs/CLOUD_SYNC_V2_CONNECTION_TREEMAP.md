...
| Fresh Apple account bootstrap | The active CloudKit branch had diverged from the installed Alpha and omitted `201661268` and `b0ea2abbc`, Alpha's stale-login-attempt and Rust-handle lifetime repairs. The earlier correct SMS code was accepted, after which the regressed controller received an HTML-shaped response that the old client tried to deserialize directly as a plist. The isolated Windows harness independently completed a semantic pull with 8,377 retained records from its previously authorized profile, isolating the failure to fresh bootstrap rather than relay reachability or general CloudKit authority. Canary now restores Alpha's exact guarded login controller while retaining the content-free SRP response classifier. | Regression identified; Alpha parity and focused tests pass; live retest pending | Run the signed Canary once. If the HTML response recurs under the Alpha-parity controller, classify the safe page metadata and route a recognized terms/account-repair step through the existing update-account UI before retrying SRP. |
