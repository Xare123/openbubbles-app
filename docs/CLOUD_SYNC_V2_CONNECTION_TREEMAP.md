...
| Relay activation and saved identity | Alpha's existing relay health check is green. A consumed sharing code is rejected by the registration relay with HTTP 401, while a fresh iPhone-relay code entered into the signed Canary was accepted and advanced through Apple password login to SMS 2FA. The OABS/QR exporter deliberately supports only `MacOSConfig`; `RelayConfig` has no encoded transfer payload. A Windows `hw_info.plist` also contains a platform-keystore-bound NGM identity and cannot be treated as a portable Android login bundle. | `LIVE-PROVEN` through fresh Canary relay admission | Preserve the admitted Canary identity while qualifying the post-2FA Apple bootstrap state. |
| Fresh Apple account bootstrap | The correct SMS code was accepted, after which Canary's mandatory fresh SRP login received an HTML-shaped response that the old client tried to deserialize directly as a plist. The isolated Windows harness independently completed a semantic pull with 8,377 retained records from its previously authorized profile, isolating the failure to fresh post-2FA bootstrap rather than relay reachability or general CloudKit authority. The repaired parser now records only SRP stage, HTTP status, content type, query-free Apple host/path, safe page class/title, byte count, and a truncated digest; it records no body, account identifier, code, cookie, or token. | `LIVE-PROVEN` boundary; diagnostic build pending | Run the exact signed Canary diagnostic once, classify the returned page, and route a recognized terms/account-repair step through the existing update-account UI before retrying SRP. |
...
| Candidate qualification | GCE run `33567074149` tested exact source `8e0e4c5d69a16e6eb069fe55f508e203e05534b6`: generated bindings, full Dart, Rust, rustpush, protector, native APK verification, GitHub-hosted signing, runner deregistration, and VM deletion passed in about 24 minutes. That signed Canary is the installed baseline and proved fresh relay admission. The HTML-response diagnostic is newer and therefore requires its own exact-SHA qualification. | `LIVE-PROVEN` installed baseline; current diagnostic unqualified | Freeze the diagnostic SHA and rerun the same signed Canary gate before in-place installation. |
