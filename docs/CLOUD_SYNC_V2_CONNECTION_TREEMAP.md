...
| Pending qualification | Database-backed Dart execution for `731988a3d` is pending. Local compilation reached the tests, but setup failed because `objectbox.dll` was absent (126); no database test passed locally. 27 composition contracts pass and changed-file analysis has no errors/warnings, with four unrelated existing style infos. Prior exact native base `35551340c` passed app Rust run `34439315172` (377 tests) and rustpush run `34439316836` (260 tests). Both completed cleanup; independent GCE and GitHub inventories are empty. |
