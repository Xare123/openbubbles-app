...
and contradictory record/type shapes. The exact-applied floor may advance only
after each change is projected. The independent fetch token may advance after
the complete journal is durably terminal, including reviewed
`retainedUnprojected` rows that remain eligible for projection repair. Generic
quarantine remains durable evidence and a fetch barrier until reviewed recovery
