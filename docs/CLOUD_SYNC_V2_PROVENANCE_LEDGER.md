...
| 37 | Summary `otr` covers original body parts, not only edited parts; historical ranges can exceed shortened current text. A local record's range length matched its first history body while exceeding its current body. | [imessage-exporter edit model/parser](https://github.com/ReagentX/imessage-exporter/blob/4d90fc8d20a745c0a8acc1e01c0631c4bab89cb4/imessage-database/src/message_types/edited.rs), independently confirmed by bounded local reason-only probing | GPL-3.0; user-authorized local evidence | Protocol facts only. No source or fixtures copied; tests use independently written synthetic messages. | `decode_message_summary` no longer constrains historical range ends by current text length or requires every original part to be edited. Arithmetic/size/key/conflict checks remain; unprojected metadata stays protected. |
