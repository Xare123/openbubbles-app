...
| FaceTime media evidence | Await the diagnostic Promise through a bounded request-scoped mailbox; only the current top-level trusted Apple origin and current call UUID may update connected state. Never substitute local preview for remote media evidence. The embedded Apple client requires DOM storage and asynchronous media playback without a second gesture. Derive visible connection status from ICE and remote-media evidence, and keep native hang-up available. |
