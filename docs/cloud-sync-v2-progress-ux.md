...
- PCS preparation now holds the existing identity-maintenance interlock and revalidates account, service-close, fence, and optional foreground continuation after asynchronous steps. PCS operation deadlines are not native cancellation: after a timeout, ownership is retained until the native future settles, and the caller still receives the fixed timeout-derived failure. A stuck native future intentionally blocks explicit teardown rather than releasing handles beneath it. Clique reset remains unreachable; no unknown join is automatically retried.
