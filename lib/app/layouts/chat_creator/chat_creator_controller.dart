...
class ChatCreatorController extends StatefulController {
  ...
  bool get canCreateGroupChats => BackendSvc.canCreateGroupChats();
  ...
  void onInit() {
    ...
    textController.supportsFormatting = selectedService.value == ChatServiceType.iMessage;
  ...
  Future<void> addressOnSubmitted() async {
    ...
    if (text.isEmail || text.isPhoneNumber) {
      ...
      await addSelected(SelectedContact(displayName: address, address: address));
  ...
  Future<void> sendMessage(BuildContext context, {String? effectId}) async {
    ...
    if (resolvedChat == null) {
      ...
      if (selectedService.value == ChatServiceType.sms && SettingsSvc.settings.smsForwardingTargets.keys.isEmpty) {
        if (kIsDesktop) {
          showSnackbar("Someone's not on iMessage!", "Enable SMS forwarding on another device!");
          return;
        }

        if (!SettingsSvc.settings.warnedTextChats.value && Get.context != null) {
          await showDialog(
            context: Get.context!,
      ...
      try {
        ...
        if (serverChat == null) {
          ...
          serverChat = await BackendSvc.createChat(
            participants.map((i) => RustPushBBUtils.formatAddress(i)).toList(),
            textController.getFinalAnnotations(),
            method,
          );
          serverChat.senderIsKnown = true;
          serverChat = await serverChat.saveAsync(updateSenderIsKnown: true);
    ...
    if (BackendSvc is RustPushBackend && initialAttachments.isEmpty && (initialText?.isEmpty ?? true)) {
      ...
      await chat.saveAsync(updateUsingHandle: true);
    }
    ...
    if (hasContent && !messageSentWithChat) {
      activeCVC.pendingSend = SendData(
        ...
        text: capturedText,
        annotations: activeCVC.buildAttributedBody(),
        subject: '',
