...
class FindMyMapWidget extends StatelessWidget {
  ...
  Widget _buildMarkerPopup(BuildContext context, Marker marker) {
    ...
    if (key?.value.contains("device")) {
      final prefix = key!.value.toString().replaceFirst("device-", "");
      final item = controller.devices.firstWhere((e) => e.id == prefix);
      return _buildDevicePopup(context, item);
    } else {
      final prefix = key!.value.toString().replaceFirst("friend-", "");
      final item = controller.friends.firstWhere((e) => e.handle?.uniqueAddressAndService == prefix);
