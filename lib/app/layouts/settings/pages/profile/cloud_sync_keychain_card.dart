...
class _CloudSyncKeychainCardState extends State<CloudSyncKeychainCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(child: Padding(padding: const EdgeInsets.all(16), child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('iCloud Keychain code', style: theme.textTheme.titleLarge), const SizedBox(height: 8), const Text('Protects your iCloud data. You may need it when signing in on a new device or recovering access. This is not your phone unlock code, Apple Account password, or temporary verification code.'), const SizedBox(height: 12), Semantics(label: statusLabel, container: true, child: ExcludeSemantics(child: Text(statusText))), const SizedBox(height: 8), Text(codeLabel), const SizedBox(height: 8), Row(children: [if (!checking && ready == null) TextButton(onPressed: refreshReadiness, child: const Text('Check again')), TextButton(onPressed: _canChange ? _openChange : null, child: _changing ? const Text('Working...') : const Text('Change code'))])]))));
  }
}
