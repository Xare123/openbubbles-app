                          ...
                          child: ListTile(
                            ...
                            title: Text("Create New",
                                style: context.theme.textTheme.bodyLarge!
                                    .copyWith(color: context.theme.colorScheme.primary)),
                            subtitle:
                                Text("Only attachments under 16mb saved", style: context.theme.textTheme.bodySmall),
                            leading: Container(
                              width: 40 * SettingsSvc.settings.avatarScale.value,
                              height: 40 * SettingsSvc.settings.avatarScale.value,
                              decoration: BoxDecoration(
                            ...
                            onTap: () async {
                              ...
                              try {
                                ...
                                for (final att in Database.attachments.getAll()) {
                                  ...
                                  if (file.existsSync() && file.lengthSync() < 16777216) {
                                ...
                                String filePath = join(
                                  downloadsDir,
                                  "BlueBubbles-chats-${now.year}${now.month}${now.day}_${now.hour}${now.minute}${now.second}.json",
                                );
                                if (kIsDesktop) {
                                ...
                                await file.create(recursive: true);
                                final jsonString = const Utf8Encoder().convert(jsonStr);
                                await file.writeAsBytes(int32BigEndianBytes(jsonString.length), mode: FileMode.write);
                                await file.writeAsBytes(jsonString, mode: FileMode.append);
                                for (final att in attMap) {
                                  final attFile = File(att);
                                  await file.writeAsBytes(int32BigEndianBytes(attFile.lengthSync()),
                                ...
                                Logger.error("Chat backup export error", error: e, trace: s);
                                Get.back(closeOverlays: true);
                                    ...
                                    onYes: () async {
                                      ...
                                      try {
                                        ...
                                        try {
                                          ...
                                          final file = res.files.first.readStream!;
                                          final chunked = ChunkedStreamReader(file);

                                          int listToInt(Uint8List list) {
                                            final byteData = ByteData.sublistView(list);
                                            return byteData.getUint32(0, Endian.big);
                                          }

                                          final size = listToInt(await chunked.readBytes(4));
                                          final jsonString = const Utf8Decoder().convert(await chunked.readBytes(size));
                                          final List<File> files = [];
                                          for (int i = 0; true; i++) {
                                            final sizeList = await chunked.readBytes(4);
                                            if (sizeList.isEmpty) break;
                                            final itemSize = listToInt(sizeList);
                                            final data = await chunked.readBytes(itemSize);
                                            final tempFile =
                                                File(join(FilesystemSvc.sysTempPath, "restorefile", i.toString()));
                                            await tempFile.create(recursive: true);
                                            files.add(tempFile);
                                            await tempFile.writeAsBytes(data);
                                          }

                                          final json = jsonDecode(jsonString) as Map<dynamic, dynamic>;
                                          ...
                                          for (final usedChat in json["chats"]) {
                                            final chat = Chat.fromMap((usedChat as Map).cast<String, dynamic>());
                                            chat.id = null;
                                            chat.save();
                                            await ChatsSvc.addChat(chat, immediate: true);
                                          }
                                          for (final usedMessage in json["messages"]) {
                                            final msg = Message.fromMap((usedMessage as Map).cast<String, dynamic>());
                                            msg.id = null;
                                            msg.save(chat: Chat.findOne(guid: usedMessage["chat"] as String?));
                                          }
                                          for (final myAtt in json["atts"]) {
                                            final raw = (myAtt as Map).cast<String, dynamic>();
                                            final att = Attachment.fromMap(raw);
                                            att.id = null;
                                            if (raw.containsKey("bytes_id")) {
                                              final restoredFile = File(att.path);
                                              await restoredFile.create(recursive: true);
                                              await files[raw["bytes_id"] as int].rename(att.path);
                                            }
                                            att.save(null);
                                          }
                                          Get.back(closeOverlays: true);
