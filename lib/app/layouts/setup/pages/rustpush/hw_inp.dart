...
class HwInpState extends State<HwInp> with ThemeHelpers {
  ...
  Future<void> handleBeeper(String code) async {
    ...
    try {
      ...
      final response2 = await HttpSvc.dio.post("$relayHost/api/v1/bridge/get-version-info",
          data: {},
          options: Options(
            headers: {
      ...
      if (response2.data["versions"]["software_name"] == "iPhone OS") {
        ...
        parsed =
            await api.configFromRelay(code: code, host: relayHost, token: "5c175851953ecaf5209185d897591badb6c3e712");
        usingBeeper = false;
      } else {
        final response = await HttpSvc.dio.post("$relayHost/api/v1/bridge/get-validation-data",
            data: {},
            options: Options(
              headers: {
                // not a secret; burner account
        ...
        parsed = await api.configFromValidationData(
            data: base64Decode(response.data["data"]),
            extra: api.HwExtra(
  ...
  Future<void> handleOpenAbsinthe(String code) async {
    ...
    try {
      ...
      final response = await HttpSvc.dio.get("$rpApiRoot/$hash",
          options: Options(
            headers: {"X-OpenBubbles-Get": ""},
          ));
      timer.cancel();
  ...
  Future<void> checkCode(String text) async {
    ...
    if (text == "testing-please-letmein") {
      ...
      try {
        await api.configureAppReview(path: PushSvc.statePath);

        controller.destroyConnection();

        var data = (await api.SharedPushState.restore(path: PushSvc.statePath))!;
        if (Platform.isAndroid) {
          ...
          PushSvc.doPoll(data.$2, pollState);
        }
    ...
    try {
      ...
      if (String.fromCharCodes(data).startsWith("OABS")) {
        ...
        var parsed = await api.configFromValidationData(
            data: data,
            extra: api.HwExtra(
                version: "13.6.4",
                protocolVersion: 1660,
  ...
  Future<T> wrapSubscriptionPromise<T>(Future<T> inner) async {
    ...
    try {
      ...
      PushSvc.mixpanel?.track("hosted-device-failure");
      rethrow;
    }
    Get.back();
    return result;
  ...
  Future<void> handleSubscriptionToken(String subscription) async {
    ...
    var activated = await HttpSvc.dio
        .post("https://hw.openbubbles.app/ticket/$token/activate", data: {"purchase_token": subscription});
  ...
  Widget materialButton(Widget inner, bool selected, void Function() onTap) {
    ...
    return Material(
      ...
      child: InkWell(
          onTap: onTap,
          borderRadius: BorderRadius.circular(15),
          child: Padding(padding: const EdgeInsets.all(17), child: inner)),
    );
  }
          ...
          child: RichText(
            text: TextSpan(
                ...
                children: staging != null
                    ? [
                        const TextSpan(text: "This device has no access to your Apple account or messages."),
                      ]
                    : [
                        const TextSpan(
                            text:
                                "To activate iMessage, Apple requires a physical iDevice to validate your sign in. Bring your own or use one of ours."),
                      ]),
          ),
        ),
      ),
            ...
            child: Theme(
              ...
              child: Column(
                children: [
                  ...
                  if (!stagingNonInp) const SizedBox(height: 20),
                  if (!stagingNonInp && stagingInfo == null)
                    Row(
                      children: [
                        Obx(() => Expanded(
                            child: controller.availableIAP.value != null
                                ? materialButton(
                                    Row(
                                      children: [
                                        Expanded(
                                            ...
                                            children: [
                                              Text("Hosted",
                                                  style: context.theme.textTheme.titleMedium!.copyWith(
                                                      color: !hosted ? null : context.theme.colorScheme.onPrimary)),
                                              Text(
                                    ...
                                    hosted, () async {
                                    if (hosted) {
                                      PushSvc.mixpanel?.track("choose-hosted");
                                      wrapSubscriptionPromise<void>((() async {
                                        await controller.ensureToken();
                                        PushSvc.client!.runWithClientNonRetryable<void>((client) async {
                                          ...
                                          client.launchBillingFlow(
                                              product: 'monthly_hosted', offerToken: d.offerIdToken);
                                        });
                                      })());
                                    }
                                    setState(() {
                                        ...
                                        child: Padding(
                                          ...
                                          child: Row(children: [
                                            Expanded(
                                              ...
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                Text("Hosted", style: context.theme.textTheme.titleMedium!),
                                                if (!kIsDesktop)
                  ...
                  if (!stagingNonInp && stagingInfo == null) const SizedBox(height: 15),
                  if (!stagingNonInp && stagingInfo == null)
                    Row(
                      children: [
                        Expanded(
                                  ...
                                  children: [
                                    Text("Self-hosted",
                                        style: context.theme.textTheme.titleMedium!
                                            .copyWith(color: hosted ? null : context.theme.colorScheme.onPrimary)),
                                    Text(
                                        "One-time access to a Mac or a supported, always-online iPhone required. Mac devices cannot use this phone's number.",
                                        style: context.theme.textTheme.bodySmall!
                                            .copyWith(color: hosted ? null : context.theme.colorScheme.onPrimary)),
                                  ],
                                ),
                  ...
                  if (!stagingNonInp && stagingInfo == null) const SizedBox(height: 10),
                  ...
                  if (!stagingNonInp && stagingInfo == null)
                    Padding(
                      ...
                      child: Text(
                          "Both options are open-source and secure. Your messages go straight to Apple servers. The device you use has no access to your account or messages.",
                          style: context.theme.textTheme.bodySmall!),
                    ),
                      ...
                      Container(
                        ...
                        child: ElevatedButton(
                          ...
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Icon(stagingNonInp ? Icons.close : Icons.arrow_back,
                      ...
                      if (staging != null || kIsDesktop || loading)
                        Container(
                          decoration: BoxDecoration(
                            ...
                            gradient: LinearGradient(
                              begin: AlignmentDirectional.topStart,
                              colors: loading || (kIsDesktop && staging == null)
                          ...
                          child: ElevatedButton(
                              ...
                              onPressed: loading || (kIsDesktop && staging == null)
                                  ? null
                                  : () async {
  ...
  Future<void> connect(api.JoinedOsConfig config, {bool isHosted = false}) async {
    ...
    try {
      if (!alreadyActivated) {
        ...
        await SettingsSvc.settings.saveOneAsync('cachedCodes');
        if (isHosted) {
          PushSvc.mixpanel?.track("hosted-device-configured");
        }
      }
      ...
      controller.supportsPhoneReg.value =
          state.name.contains("iPhone") || state.name.contains("iPod") || state.name.contains("iPad");
