...
class Database {
  ...
  static late final Box<CloudProtectedPageLeaseEntity> cloudProtectedPageLeases;
  ...
  static Future<void> init({bool cloudSyncV2Harness = false}) async {
    ...
    if (!kIsDesktop) {
      ...
      await _initDatabaseDesktop(cloudSyncV2Harness: cloudSyncV2Harness);
    }
    ...
    try {
      ...
      Database.cloudProtectedPageLeases = store
      ...
      Database.cloudAttachmentMaterializations = store
          .box<CloudAttachmentMaterializationEntity>();
      Database.cloudSemanticSnapshots = store
          .box<CloudSemanticSnapshotEntity>();
      Database.cloudSemanticReplay = store.box<CloudSemanticReplayEntity>();
      Database.cloudKitV2QuarantineRepairs = store
          .box<CloudKitV2QuarantineRepairReceiptEntity>();
      Database.cloudKitDeletionIntents = store
          .box<CloudKitDeletionIntentEntity>();
      Database.cloudKitDeletionQuarantines = store
          .box<CloudKitDeletionQuarantineEntity>();
      ...
      if (cloudSyncV2Harness) rethrow;
    }

    if (!cloudSyncV2Harness) {
      try {
      ...
      try {
        ...
        Logger.error(
          "Failed to perform database migrations!",
          error: e,
          trace: s,
        );
      }
  ...
  static Future<void> _initDatabaseMobile({bool? storeOpenStatus}) async {
    Directory objectBoxDirectory = Directory(
      join(fs.appDocDir.path, 'objectbox'),
    );
    final isStoreOpen =
  ...
  static Future<void> _initDatabaseDesktop({
    bool cloudSyncV2Harness = false,
  }) async {
    Directory objectBoxDirectory = Directory(
      join(fs.appDocDir.path, 'objectbox'),
    );
    ...
    try {
      ...
      if (!cloudSyncV2Harness &&
        ...
        Directory oldCustom = Directory(
          join(ss.prefs.getString('custom-path')!, 'objectbox'),
        );
      ...
      Logger.error(
        "Failed to initialize desktop database!",
        error: e,
        trace: s,
      );
      if (cloudSyncV2Harness) rethrow;
  ...
  static void _performDatabaseMigrations({int? versionOverride}) {
    ...
    switch (Database.version) {
      ...
      case 2:
        Logger.info(
          "Fetching all messages and handles...",
          tag: "DB-Migration",
        );
        ...
        if (messages.isNotEmpty) {
          ...
          Logger.info(
            "Replacing handleIds for messages...",
            tag: "DB-Migration",
          );
    ...
    s.stop();
    Logger.info(
      "Completed database migration in ${s.elapsedMilliseconds}ms",
      tag: "DB-Migration",
    );
  }
