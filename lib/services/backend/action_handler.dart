...
class ActionHandler extends GetxService {
  ...
  Future<void> handleIncomingFaceTimeCall(Map<String, dynamic> data) async {
    ...
    await NotificationsSvc.createIncomingFaceTimeNotification(callUuid, caller, link, chatIcon, poster);
  }
  ...
  Future<void> handleIncomingFaceTimeCallLegacy(Map<String, dynamic> data) async {
    ...
    if (address != null) {
      ...
      caller = contact?.displayName ?? caller;
      await NotificationsSvc.createIncomingFaceTimeNotification(null, caller!, "", chatIcon, null);
    }
  }
  ...
  Future<void> handleEvent(String event, Map<String, dynamic> data, String source, {bool useQueue = true}) async {
    ...
    switch (event) {
      ...
      case "typing-indicator":
        ...
        if (chat != null) {
          // Legacy socket typing indicators are intentionally ignored here.
          // Current typing state is driven by the RustPush handle-based path.
        }
        return;
