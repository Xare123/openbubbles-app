...
import 'cloud_sync_windows_dev_profile.dart';
...
class FilesystemService extends GetxService {
  ...
  Directory? _configuredAppDocDir;
  bool cloudSyncV2WindowsDevProfileActive = false;
  ...
  /// Selects the fixed Cloud Sync V2 Windows profile before service startup.
  /// Ordinary app entry points never call this method.
  void configureCloudSyncV2WindowsDevProfile() {
    if (kIsWeb || !Platform.isWindows) {
      throw StateError('cloud_sync_windows_dev_profile_windows_required');
    }
    final directory = CloudSyncWindowsDevProfile.requireBootstrapped();
    _configuredAppDocDir = directory;
    // The dedicated harness writes startup diagnostics before the ordinary
    // asynchronous filesystem initialization runs. Assign the already
    // validated fixed directory here as well so that path is available from
    // the first startup stage.
    appDocDir = directory;
    cloudSyncV2WindowsDevProfileActive = true;
  }
  ...
  Future<void> init({bool headless = false}) async {
    if (!kIsWeb) {
      ...
      appDocDir =
          _configuredAppDocDir ??
          (kIsDesktop
                  ? await getApplicationSupportDirectory()
                  : await getApplicationDocumentsDirectory())
              as Directory;
      if (kIsDesktop && Platform.isWindows) {
        final String appDataRoot = joinAll(
          split(appDocDir.absolute.path).slice(0, 4),
        );
        final Directory msStoreLocation = Directory(
          join(
            appDataRoot,
            "Local",
            "Packages",
        ...
        if (_configuredAppDocDir == null &&
            !appDocDir.existsSync() &&
            msStoreLocation.existsSync()) {
      ...
      if (!headless) {
        ...
        final file2 = await rootBundle.load(
