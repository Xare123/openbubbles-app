...
class OutgoingQueue extends Queue {
  ...
  Future<dynamic> prepItem(QueueItem _) async {
    ...
    switch (item.type) {
      ...
      case QueueType.sendMessage:
        return await OutgoingMsgHandler.prepMessage(
          item.chat,
          item.message,
          item.selected,
          item.reaction,
      ...
      case QueueType.sendAttachment:
        return await OutgoingMsgHandler.prepAttachment(item.chat, item.message);
  ...
  Future<void> handleQueueItem(QueueItem _) async {
    ...
    switch (item.type) {
      case QueueType.sendMessage:
        await handleSend(
          () => OutgoingMsgHandler.sendMessage(item.chat, item.message, item.selected, item.reaction),
          item.chat,
        );
        break;
      case QueueType.sendAttachment:
        await handleSend(
          () => OutgoingMsgHandler.sendAttachment(item.chat, item.message, item.customArgs?['audio'] ?? false),
          item.chat,
        );
        break;
      case QueueType.newMessage:
