...
class MessagesService extends GetxController {
  ...
  void updateMessage(Message updated, {String? oldGuid}) {
    ...
    final isEdited = updated.dateEdited != toUpdate.dateEdited;
  ...
  Future<void> editMessage(Message message, int partIndex, AttributedBody newText) async {
    ...
    final List<Annotation>? originalAnnotations =
        partIdx >= 0 ? preState!.parts[partIdx].annotations.map((a) => a.copy()).toList() : null;
    final DateTime? originalDateEdited = preState?.dateEdited.value;
    if (preState != null) {
      if (partIdx >= 0) {
        preState.parts[partIdx].text = newText.string;
        preState.parts[partIdx].annotations = newText.runs
            .map((run) => Annotation(
                  range: [run.range[0], run.range[0] + run.range[1]],
                  mentionedAddress: run.attributes?.mention,
    ...
    try {
      ...
      if (postState != null) {
        ...
        if (partIdx >= 0 && originalText != null) {
          ...
          postState.parts[partIdx].annotations = originalAnnotations?.map((a) => a.copy()).toList() ?? <Annotation>[];
  ...
  Future<void> unsendMessage(Message message, MessagePart part) async {
    final messageGuid = message.guid;
    if (messageGuid == null) return;

    // --- Optional optimistic update (state may not exist) ---
    final preState = getMessageStateIfExists(messageGuid);
    final partIdx = preState?.parts.indexWhere((p) => p.part == part.part) ?? -1;
    final DateTime? originalDateEdited = preState?.dateEdited.value;
    ...
    try {
      final updatedMessage = await BackendSvc.unsend(message, part);
