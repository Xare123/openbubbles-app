...
use crate::cloud_sync_extension_metadata::{validate_bundle_id, validate_metadata};
use crate::cloud_sync_extension_metadata::{
    ExtensionBalloonMetadata, ExtensionPayloadFailure, ExtensionPayloadMetadata,
    ExtensionTemplateMetadata, MAX_ARCHIVE_OBJECTS, MAX_COMPRESSED_ICON_BYTES, MAX_DEPTH,
    MAX_EXPANDED_BYTES, MAX_ICON_BYTES, MAX_LIVE_LAYOUT_BYTES, MAX_PAYLOAD_BYTES, MAX_STRING_BYTES,
    MAX_VISITED_NODES,
};
...
fn icon_encoding(bytes: &[u8]) -> &'static str {
...
fn decode_icon(bytes: &[u8]) -> Result<Vec<u8>> {
    ...
    let encoding = icon_encoding(bytes);
    log::debug!(target: "rust_lib_bluebubbles::cloud_sync_transient_bridge",
        "CloudKit V2 extension icon encoding={encoding} bytes={}", bytes.len());
    // Cloud archive Data can already contain image bytes. This is byte routing,
    // not raster validation; the existing image renderer still decodes them.
    // Keep the same input cap, and never treat arbitrary non-gzip data as an icon.
    match encoding {
        "png" | "jpeg" | "gif" | "webp" => return Ok(bytes.to_vec()),
        "gzip" => {}
        _ => return Err(Failure::InvalidIcon),
    }
    ...
    decoder
        ...
        .map_err(|_| {
            log::debug!(target: "rust_lib_bluebubbles::cloud_sync_transient_bridge",
                "CloudKit V2 extension icon gzip_failure=decode_or_integrity");
            Failure::InvalidIcon
        })?;
    ...
    if !trailing.is_empty() {
        log::debug!(target: "rust_lib_bluebubbles::cloud_sync_transient_bridge",
            "CloudKit V2 extension icon gzip_failure=trailing bytes={} all_zero={}",
            trailing.len(), trailing.iter().all(|byte| *byte == 0));
...
mod tests {
    use super::*;
    use crate::cloud_sync_extension_metadata::{
        parse_generated_metadata_json, parse_projection_metadata_json,
        ...
        MAX_BUNDLE_ID_BYTES, MAX_JSON_BYTES,
    };
    ...
    #[test]
    fn recognized_raw_icon_bytes_are_preserved_without_a_gzip_roundtrip() {
        // Reuse the neutral 1x1 PNG from the Windows attachment-write fixture.
        let png = base64::Engine::decode(&base64::engine::general_purpose::STANDARD,
            "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==").unwrap();
        assert_eq!(decode_icon(&png).unwrap(), png);
        assert_eq!(decode_icon(&gzip(&png)).unwrap(), png);
        let mut raw_archive = balloon();
        raw_archive
            .as_dictionary_mut()
            .unwrap()
            .insert("ai".into(), Value::Data(png.clone()));
        assert_eq!(decode(raw_archive).unwrap().balloon.icon.unwrap(), png);
        for bytes in [
            &b"\x89PNG\r\n\x1a\nsynthetic-container"[..],
            &b"\xff\xd8\xffsynthetic-container"[..],
            &b"GIF89asynthetic-container"[..],
            &b"RIFF\x00\x00\x00\x00WEBPsynthetic-container"[..],
        ] {
            assert_eq!(decode_icon(bytes).unwrap(), bytes);
            let mut root = balloon();
            root.as_dictionary_mut()
                .unwrap()
                .insert("ai".into(), Value::Data(bytes.to_vec()));
            assert_eq!(decode(root).unwrap().balloon.icon.unwrap(), bytes);
        }
        // This test concerns routing only, not successful rendering of the
        // synthetic containers. Invalid/unknown byte streams stay rejected.
        assert_eq!(decode_icon(b"private-user-data"), Err(Failure::InvalidIcon));
        let mut oversized = b"\x89PNG\r\n\x1a\n".to_vec();
        oversized.resize(MAX_COMPRESSED_ICON_BYTES + 1, 0);
        assert_eq!(decode_icon(&oversized), Err(Failure::LimitExceeded));
    }
