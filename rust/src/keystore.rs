...
use keystore::{
    backup::{BackupKeystore, BackupKeystoreState},
    init_keystore, keystore,
    software::{SoftwareKeystore, SoftwareKeystoreState},
    EcCurve, EcKeystoreKey, EncryptMode, KeyType, Keystore, KeystoreAccessRules, KeystoreDeriveKey,
    KeystoreDigest, KeystoreError, KeystorePadding,
};
...
#[cfg(target_os = "android")]
static ANDROID_NATIVE_KEYSTORE: OnceLock<Arc<dyn NativeKeystore>> = OnceLock::new();

/// Returns the raw Android Keystore callback rather than the software or
/// backup wrapper selected for the legacy rustpush keystore.
#[cfg(target_os = "android")]
pub(crate) fn android_native_keystore() -> Option<&'static Arc<dyn NativeKeystore>> {
    ANDROID_NATIVE_KEYSTORE.get()
}
...
pub fn supports_import(keystore: &NativeKeystoreHolder) -> Result<bool, KeystoreError> {
    if !keystore
        .keystore
        .supports_import()
        .map_err(|e| <NativeKeystoreError as Into<KeystoreError>>::into(e))?
    {
        return Ok(false);
    }
    ...
    keystore.import_key(
        "test:ec",
        KeyType::Ec(EcCurve::P384),
        &test_ec.private_key_to_der()?,
        KeystoreAccessRules {
            can_agree: true,
    ...
    let ephermeral_sender =
        other_key
            .public_key()
            .to_bytes(&curve, PointConversionForm::UNCOMPRESSED, &mut context)?;
...
pub fn setup_keystore(dir: String, keystore: Arc<dyn NativeKeystore>) {
    ...
    #[cfg(target_os = "android")]
    let _ = ANDROID_NATIVE_KEYSTORE.set(keystore.clone());

    let keystore = NativeKeystoreHolder { keystore };
    ...
    if has_support {
        let (state, key) = if let Ok(existing) = plist::from_file(&keystore_path) {
            ...
            let (keystore, key) =
                BackupKeystoreState::new(&keystore).expect("Failed to initialize keystore!");
            (keystore, Some(key))
        };
...
pub trait NativeKeystore: Send + Sync + Debug + 'static {
    ...
    fn sign(
        ...
        data: Vec<u8>,
    ) -> Result<Vec<u8>, NativeKeystoreError>;
    fn verify(
        &self,
        alias: String,
        digest: KeystoreDigest,
...
impl AuthorizationList {
    fn from_auth_rules(r#type: KeyType, rules: &KeystoreAccessRules) -> Self {
        ...
        Self {
            ...
            block_mode: if rules.block_modes.is_empty() {
                None
            } else {
                Some(
                    rules
            ...
            digest: if rules.digests.is_empty() {
                None
            } else {
                Some(
                    rules
                        .digests
                        .iter()
                        .map(|m| Digest::from_keystore(m) as u32)
                        .collect(),
                )
            },
            min_mac_length: if rules
                .block_modes
                .iter()
                .any(|b| matches!(b, EncryptMode::Gcm))
            {
                Some(128)
            } else {
                None
            },
            padding: if rules.signature_padding.is_empty() && rules.encryption_paddings.is_empty() {
                Some(std::iter::once(PaddingMode::None as u32).collect())
            } else {
                Some(
                    rules
                        .signature_padding
                        ...
                        .chain(rules.encryption_paddings.iter().map(|m| match m {
                            KeystorePadding::None => PaddingMode::None,
                            KeystorePadding::OAEP { .. } => PaddingMode::RsaOaep,
            ...
            mgf_digest: if rules.mgf1_digests.is_empty() {
                None
            } else {
                Some(
                    rules
            ...
            user_auth_type: if rules.require_user {
                Some(
...
fn wrap_import_key(
    ...
    let tag = gcm
        .encrypt_in_place_detached(
            &Nonce::from_slice(&iv),
            &rasn::der::encode(&desc).expect("Failed to encode?"),
            &mut priv_encrypted,
        )
...
impl Keystore for NativeKeystoreHolder {
    fn create_key(
        ...
        self.keystore
            .create_key(alias.to_owned(), r#type, access_rules)
            .map_err(|e| e.into())
    }
    ...
    fn import_key(
        ...
        info!("Keystore importing protected key");
        let rsa = Rsa::public_key_from_der(
            &self
                .keystore
                .get_import_wrap_key()
                .map_err(|e| <NativeKeystoreError as Into<KeystoreError>>::into(e))?,
        ...
        info!("Prepared wrapped key material (bytes={})", wrapped.len());

        self.keystore
            .import_key(alias.to_owned(), r#type, wrapped, access_rules)
            .map_err(|e| e.into())
    }
    ...
    fn sign(
        ...
        self.keystore
            .sign(alias.to_owned(), digest, padding, data.to_vec())
            .map_err(|e| e.into())
    }
    ...
    fn derive(&self, alias: &str, peer: &[u8]) -> Result<Vec<u8>, KeystoreError> {
        ...
        self.keystore
            .derive(alias.to_owned(), pub_key.public_key_to_der()?)
            .map_err(|e| e.into())
    }
    ...
    fn encrypt(
        ...
        self.keystore
            .encrypt(alias.to_owned(), plaintext.to_vec(), *mode)
            .map_err(|e| e.into())
    }
    ...
    fn decrypt(
        ...
        self.keystore
            .decrypt(alias.to_owned(), ciphertext.to_vec(), *mode)
            .map_err(|e| e.into())
    }
}
