...
use rustpush::{
    get_gateways_for_mccmnc,
    passwords::{
        Passkey, PasskeyCriteria, PasswordCriteria, PasswordManager, PasswordManagerMeta,
        PasswordManagerMetaChange, PasswordManagerMetaData, PasswordManagerMetaDataCtx,
        PasswordRawEntry,
    },
    EntitlementAuthState, GenerateVerificationTokenRequest, PushError,
};
use tokio::{
...
use crate::{
    api::api::{
        approve_circle, decline_facetime, do_first_time_init, get_2fa_code, get_entitlements,
        recv_wait, set_status, teardown_2fa, APSWatcher, DaemonData, PollResult, PushMessage,
        SharedPushState,
    },
...
pub fn start(
    dir: String,
    packager: Arc<dyn KotlinFilePackager>,
    wifi: Arc<dyn HandleWifiNetworksCallback>,
) {
...
pub fn init_native(dir: String, handle: Option<String>, handler: Arc<dyn MsgReceiver>) {
    ...
    RUNTIME.spawn(async move {
        ...
        let result = if let Some(handle) = handle {
            ...
            SharedPushState::restore(dir).await.map(|a| {
...
impl NativePushState {
    pub fn start_loop(self: Arc<NativePushState>, handler: Arc<dyn MsgReceiver>) {
        RUNTIME.spawn(async move {
            ...
            loop {
                match std::panic::AssertUnwindSafe(recv_wait(&mut watcher, &self.state))
                    .catch_unwind()
                    .await
                {
                    Ok(yes) => {
                    ...
                    Err(payload) => {
                        ...
                        if panic.map(is_terminal_poll_panic).unwrap_or(false) {
                            warn!(
                                "Stopping APS receive loop after terminal watcher panic: {:?}",
                                panic
                            );
                            break;
                        }
    ...
    pub fn get_available_groups(&self, groups_callback: Arc<dyn AvailableGroupsCallback>) {
        ...
        RUNTIME.spawn(async move {
            ...
            let result = state
                .groups
                .iter()
                .filter_map(|g| Some((g.1.share.as_ref()?.display_name.clone(), g.0.clone())))
                .collect::<HashMap<_, _>>();
            groups_callback.groups(result);
        });
    }

    pub fn keychain_password_insert(
        &self,
        site: String,
        user: String,
        password: String,
        callback: Arc<dyn InsertKeychainCallback>,
        group: Option<String>,
    ) {
        let passwords = self
            .state
        ...
        RUNTIME.spawn(async move {
            ...
            let a = passwords
                .modify_password_entry::<PasswordRawEntry>(
                    &criteria,
                    |pw| {
                ...
                .err();
            let b = passwords
                .modify_password_entry::<PasswordManagerMeta>(
                    &criteria,
                    |pw| {
    ...
    pub fn keychain_passkey_insert(
        &self,
        site: String,
        record_id: String,
        id: Vec<u8>,
        ...
        RUNTIME.spawn(async move {
            ...
            callback.keys(
                ...
                passwords
                    .passkeys
                    .into_iter()
                    .map(|(k, p)| SavedPasskey {
                        cred_id: k,
                        id: p.klbl.clone(),
                        tag: p.atag.clone(),
                        key: PKey::from_ec_key(p.get_key())
                            .unwrap()
    ...
    pub fn do_special_apple_auth(
        &self,
        client_data_hash: String,
        callback: Arc<dyn SpecialAppleAuthCallback>,
    ) {
        let token = self
            .state
            .icloud_services
            .as_ref()
            .map(|i| i.token_provider.clone())
            .expect("no token");
        RUNTIME.spawn(async move {
            let result = std::panic::AssertUnwindSafe(token.generate_verification_token(
                GenerateVerificationTokenRequest::Passkey { client_data_hash },
            ))
            .catch_unwind()
            .await;
            match result {
                Ok(Ok(success)) => {
                    ...
                    callback.got_verification(
                        HashMap::from_iter([(key.to_string(), value.to_string())]),
                        None,
                    );
                }
                Ok(Err(e)) => {
